...
#
# This does NOT install Millennium or the Rewired plugin — use scripts/install.sh for the full stack.
...
if [[ "$FORCE" != "1" ]] && already_present; then
  ...
  info "Force reinstall: FORCE=1 curl -fsSL https://raw.githubusercontent.com/Sigmachan/STLT-Rewired/main/scripts/install-linux-unlock.sh | bash"
  exit 0
fi
