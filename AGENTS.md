...
- On a real Linux Steam desktop the plugin is supported. `scripts/install.sh` installs
  ACCELA + SLSsteam via enter-the-wired, then Millennium + plugin. Unlock-only:
  `scripts/install-linux-unlock.sh`. Shared config: `~/.local/share/Rewired/rewired.json`.
